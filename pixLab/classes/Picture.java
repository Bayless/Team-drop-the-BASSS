 ...
 public void fixUnderwater() {
  ...
  public void mirrorVertical()
  {
	...
	for (int row = 0; row < length / 2; row++)
		...
		for (int col = 0; col < pixels[0].length; col++)
			...
			downPixel = pixels[length-row-1][col];
      ...
      public void mirrorDiagonal()
  {
    ...
    for (int row = 0; row < length; row++)
    {
      ...
      {
	  if (col <= row){
	      fPixel = pixels[row][col];
	      nPixel = pixels[col][row];
	      nPixel.setColor(fPixel.getColor());
	  }
      }
    } 
  }
  ...
  public void mirrorArms()
  {
    ...
    // loop through the rows
    for (int row = 177; row < 188; row++)
    {
      // loop from 13 to just before the mirror point
      for (int col = 235; col < mirrorPoint; col++)
      {
        leftPixel = pixels[row][col];      
        rightPixel = pixels[row+17]                       
                         [col];
        rightPixel.setColor(leftPixel.getColor());
      }
    }
        for (int row = 169; row < 188; row++)
    {
      // loop from 13 to just before the mirror point
      for (int col = 100; col < 170; col++)
      {
        leftPixel = pixels[row][col];      
        rightPixel = pixels[row+20]                       
                         [col];
        rightPixel.setColor(leftPixel.getColor());
      }
    }
